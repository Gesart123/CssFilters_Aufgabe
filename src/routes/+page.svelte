<script>
	// Simple state for filter controls
	let blur = 8;
	let contrast = 100;
	let brightness = 100;
	let grayscale = 0;
	...
	const photo = 'https://images.unsplash.com/photo-1504198453319-5ce911bafcde?w=900&auto=format&fit=crop&q=80';
	const texture = 'https://images.unsplash.com/photo-1505761671935-60b3a7427bad?w=1200&auto=format&fit=crop&q=80';
...
<main class="min-h-screen bg-slate-950 text-slate-50">
	<section class="mx-auto flex max-w-6xl flex-col gap-10 px-4 py-12 sm:px-8">
		<header class="space-y-3">
			<p class="text-sm font-semibold uppercase tracking-[0.2em] text-indigo-300">CSS Filters & Blend Modes</p>
			<h1 class="text-3xl font-bold sm:text-4xl">Playground für Blur, Kontrast, Helligkeit, Grayscale & Blend Modes</h1>
			<p class="max-w-3xl text-base text-slate-200">
				Steuere mit einfachen Reglern die CSS <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">filter</code>-Property
				und probiere verschiedene <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">mix/background-blend-mode</code> Werte aus.
			</p>
		</header>

		<section class="grid gap-8 lg:grid-cols-2">
			<article class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-indigo-500/10 backdrop-blur">
				<div class="flex items-center justify-between gap-4">
					<h2 class="text-xl font-semibold">Filter Demo</h2>
					<button class="rounded-lg bg-indigo-500 px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-400" on:click={resetFilters}>
						Reset
					</button>
				</div>
				...
				<div class="mt-6 space-y-4">
					...
					<label class="grid gap-1 text-sm font-semibold">
						...
						<input class="accent-indigo-400" type="range" min="50" max="200" step="1" bind:value={contrast} />
					</label>
					<label class="grid gap-1 text-sm font-semibold">
						<span>Helligkeit ({brightness}%)</span>
						<input class="accent-indigo-400" type="range" min="50" max="200" step="1" bind:value={brightness} />
					</label>
					<label class="grid gap-1 text-sm font-semibold">
						<span>Grayscale ({grayscale}%)</span>
						<input class="accent-indigo-400" type="range" min="0" max="100" step="1" bind:value={grayscale} />
					</label>
				</div>
			...
			<article class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-purple-500/10 backdrop-blur">
				...
				<p class="mt-2 text-sm text-slate-200">
					<code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">mix-blend-mode</code> mischt Vordergrund mit dem darunterliegenden Element. <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">background-blend-mode</code> mischt mehrere Hintergründe eines Elements.
				</p>
		...
		<section class="rounded-2xl border border-white/10 bg-white/5 p-6 shadow-lg shadow-slate-900/40 backdrop-blur">
			...
			<ol class="mt-3 list-decimal space-y-2 pl-5 text-sm text-slate-200">
				...
				<li>Binde den Slider mit <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">bind:value</code> an eine Variable <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">blur</code>.</li>
				<li>Setze <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">style={`filter: blur(${'{'}blur{'}'})`}</code> auf das Bild.</li>
				<li>Füge weitere Slider für Kontrast, Helligkeit, Grayscale hinzu.</li>
				<li>Erzeuge eine zweite Karte mit zwei Hintergründen (Gradient + Bild) und wende <code class="rounded bg-slate-900 px-1 py-0.5 text-[0.9em]">background-blend-mode</code> an.</li>
